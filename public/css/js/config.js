...
function saveSession(session, role) {
  ...
  if (role) sessionStorage.setItem('gmc_role', String(role).toLowerCase());
}
...
function roleFromClinicEmail(email) {
  ...
  return ['admin', 'doctor', 'nurse', 'staff', 'patient'].includes(local) ? local : null;
}
...
function clearSession() {
  ...
  sessionStorage.removeItem('gmc_role');
}
