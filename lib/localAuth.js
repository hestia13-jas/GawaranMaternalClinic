...
function listLocalAppointments(user) {
  ...
  if (user.role === 'doctor') {
    return rows.filter((appointment) => appointment.doctor_id === user.id);
  }
  return rows;
}
...
function updateLocalAppointmentStatus({ appointmentId, status, reason, actor }) {
  const store = readStore();
  ...
  createLocalNotificationRecord(store, {
    ...
    title: status === 'confirmed' ? 'Appointment approved' : 'Appointment declined',
    message: status === 'confirmed'
      ? `Your ${appointment.type} appointment on ${new Date(appointment.date).toLocaleString()} was approved.`
      : `Your ${appointment.type} appointment was declined. Reason: ${reason}`,
    type: 'appointment',
  });
