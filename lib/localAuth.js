...
const RUNTIME_DATA_DIR = process.env.LOCAL_DATA_DIR
  || (process.env.VERCEL || process.env.LAMBDA_TASK_ROOT || process.env.AWS_LAMBDA_FUNCTION_NAME
    ? path.join(os.tmpdir(), 'gawaran-maternal-clinic')
    : BUNDLED_DATA_DIR);
let STORE_PATH = path.join(RUNTIME_DATA_DIR, 'local-users.json');
const SEED_STORE_PATH = path.join(BUNDLED_DATA_DIR, 'local-users.json');
...
function readStore() {
  try {
    const sourcePath = fs.existsSync(STORE_PATH)
      ? STORE_PATH
      : (fs.existsSync(SEED_STORE_PATH) ? SEED_STORE_PATH : null);
    if (!sourcePath) return emptyStore();
    return { ...emptyStore(), ...JSON.parse(fs.readFileSync(sourcePath, 'utf8')) };
  } catch {
    return emptyStore();
  }
}
...
function writeStore(store) {
  try {
    ...
    if (!['EROFS', 'EACCES', 'EPERM'].includes(err.code) || STORE_PATH.startsWith(os.tmpdir())) {
      throw err;
    }
    STORE_PATH = path.join(os.tmpdir(), 'gawaran-maternal-clinic', 'local-users.json');
    fs.mkdirSync(path.dirname(STORE_PATH), { recursive: true });
    fs.writeFileSync(STORE_PATH, JSON.stringify(store, null, 2));
  }
}
