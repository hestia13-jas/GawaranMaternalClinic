...
function clinicFrom() {
  return process.env.SMTP_FROM || 'Gawaran Maternal Clinic <noreply@gawaranclinic.ph>';
}
...
async function sendAppointmentEmail({ to, patientName, status, appointment, reason }) {
  ...
  const title = approved ? 'Appointment approved' : 'Appointment declined';
  ...
  const text = approved
    ? `Hello ${patientName || 'Patient'},\n\nYour ${appointment.type || 'appointment'} on ${formattedDate} has been approved.\n\nGawaran Maternal Clinic`
    : `Hello ${patientName || 'Patient'},\n\nYour ${appointment.type || 'appointment'} request was declined.\nReason: ${reason}\n\nGawaran Maternal Clinic`;
  const html = emailShell({
    title,
    subtitle: 'Appointment update',
    body: `
      <p>Hello ${patientName || 'Patient'},</p>
      <p>Your <strong>${appointment.type || 'appointment'}</strong> request for <strong>${formattedDate}</strong> has been <strong>${approved ? 'approved' : 'declined'}</strong>.</p>
      ${approved ? '<p>Please arrive 10 minutes before your scheduled time.</p>' : `<p><strong>Reason:</strong> ${reason}</p>`}
